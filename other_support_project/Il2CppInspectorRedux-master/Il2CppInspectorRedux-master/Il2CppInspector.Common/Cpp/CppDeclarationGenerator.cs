...
public class CppDeclarationGenerator
{
    ...
    public void IncludeType(TypeInfo ti)
    {
        ...
        // NIKKE has types that exceed the normal recursion limit (example: a UniTask that wraps a bunch of ValueTuples),
        // which will get caught by these checks. However, the types will just be emitted as empty structs instead of leading to a broken output.
    ...
    private CppComplexType GenerateNestingLimitExceededStruct(TypeInfo ti)
    {
        var name = TypeNamer.GetName(ti);
        var cls = types.Struct(name);
        return cls;
    }
